...
protected:
    ...
    bool process_cb(ProcessSamples::Request& request, ProcessSamples::Response& response) {
        ...
        std::vector<std::string>& objects = request.samples.order.contents;
        for(int i = 0 ; i < samples.size(); ++i) {
            ...
            for(int j = 0; j < objects.size(); ++j) {
                feature_match(objects[j], pcl_clouds[i], samples[i]);
            }
        ...
        std::vector<PointCluster> clusters = segmentCloud(out, object, objects);
